      ...
      <section className="py-24 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          ...
          >
            ...
            >
              6 Pilihan{" "}
              <span style={{ background: "linear-gradient(to right,#FF006E,#F59E0B)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                Warna Cantik!
              </span>
            </h2>
            <p className="text-gray-500 text-lg">Pilih warna yang paling cocok sama style kamu 💕</p>
          </motion.div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
            {colorOptions.map((c, i) => (
              <motion.a
                ...
                whileHover={{ y: -8, scale: 1.02 }}
                className="rounded-3xl overflow-hidden shadow-lg group cursor-pointer block"
              >
                {/* Color swatch with keycap demo */}
                <div className="h-32 relative flex items-center justify-center" style={{ background: c.bg }}>
                  <div className="flex gap-2">
                    {(["A", "B", "C"] as const).map(ch => (
                      <div
                        key={ch}
                        className="w-10 h-10 rounded-lg flex items-center justify-center font-black text-sm shadow-md"
                        style={{
                ...
                <div className="p-4 bg-white">
                  <h3
                    className="font-black text-xl text-gray-900 mb-1"
                    style={{ fontFamily: "var(--font-nunito)" }}
                  >
                    {c.emoji} {c.name}
                  </h3>
                  <p className="text-gray-400 text-xs mb-3">Keycap · Custom Nama · Clickable</p>
                  <span
                    className="block text-center text-white font-black py-2 px-4 rounded-xl text-xs transition-opacity group-hover:opacity-90"
                    style={{ background: `linear-gradient(135deg,${c.bg},#7B2FBE)` }}
                  >
                    Pesan Warna Ini 🛒
                  </span>
                </div>
