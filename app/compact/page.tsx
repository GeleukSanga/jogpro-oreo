...
export default function CompactPage() {
  ...
  return <main className="overflow-x-hidden bg-white">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 py-24 grid lg:grid-cols-2 gap-14 items-center">
        <div className="text-center lg:text-left">
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{ delay: .35 }} className="relative inline-block">
            ...
            <a href="/checkout?produk=Compact%20Cable%20Winder&harga=20000" className="inline-flex items-center gap-2 font-black text-black rounded-full px-8 py-4 shadow-2xl transition-transform hover:scale-105" style={{ background: "#FFE600", fontFamily: "var(--font-nunito)" }}>🛒 Pesan Sekarang — Rp 20.000</a>
            <p className="text-center text-sm text-yellow-200/70 mt-2">atau <a href="/checkout?produk=Compact%20Cable%20Winder&harga=20000" className="text-yellow-300 font-bold underline underline-offset-2 hover:text-yellow-100">bayar via QRIS 💳</a>{" · "}<a href={waLink("Halo Jogpro! Saya mau pesan Compact Cable Winder")} target="_blank" rel="noopener noreferrer" className="text-yellow-300 font-bold underline underline-offset-2 hover:text-yellow-100">Ada pertanyaan? Chat WA 💬</a></p>
          </motion.div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {variants.map((p, i) => <motion.div key={p.name} variants={scaleIn} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: i*.12 }} whileHover={{ y: -10, scale: 1.02 }} className="rounded-3xl overflow-hidden bg-white shadow-xl">
            ...
            <div className="p-6"><h3 className="text-2xl font-black text-gray-900 mb-1" style={{ fontFamily: "var(--font-nunito)" }}>Compact {p.name}</h3><p className="text-gray-400 text-sm mb-5">Twist-to-wind · Max kabel 1.5m</p><a href="/checkout?produk=Compact%20Cable%20Winder&harga=20000" className="block text-center text-white font-black py-3 rounded-2xl bg-gradient-to-r from-sky-500 to-pink-500">Pilih Warna Ini 🛒</a><p className="text-center text-xs text-gray-400 mt-2">atau <a href="/checkout?produk=Compact%20Cable%20Winder&harga=20000" className="text-purple-600 font-bold underline underline-offset-2 hover:text-purple-800">bayar via QRIS 💳</a>{" · "}<a href={waLink(p.text)} target="_blank" rel="noopener noreferrer" className="text-purple-600 font-bold underline underline-offset-2 hover:text-purple-800">Chat WA 💬</a></p></div>
          </motion.div>)}
        </div>
      </div>
    </section>
    ...
    <section className="py-28 relative overflow-hidden text-center" style={{ background: "linear-gradient(135deg,#00B4D8 0%,#7B2FBE 50%,#FF006E 100%)" }}><div className="max-w-3xl mx-auto px-6 relative z-10"><motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className="font-black text-white leading-tight mb-5" style={{ fontSize: "clamp(2.2rem,6vw,4.2rem)", textShadow: "3px 3px 0 rgba(0,0,0,.18)", fontFamily: "var(--font-nunito)" }}>Siap Rapikan Kabel?<br/>Yuk Order Sekarang! 🎉</motion.h2><p className="text-white/80 text-xl mb-8">Rp 20.000 · Gratis ongkir Pulau Jawa · Cocok semua kabel max 1.5m</p><a href="/checkout?produk=Compact%20Cable%20Winder&harga=20000" className="inline-flex items-center gap-3 font-black text-lg rounded-full px-10 py-5 bg-white hover:bg-yellow-300 text-black shadow-2xl">🛒 Pesan Sekarang</a><p className="text-center text-sm text-gray-300 mt-2">atau <a href="/checkout?produk=Compact%20Cable%20Winder&harga=20000" className="text-purple-300 font-bold underline underline-offset-2 hover:text-purple-200">bayar via QRIS 💳</a>{" · "}<a href={waLink("Halo Jogpro! Saya mau pesan Compact Cable Winder")} target="_blank" rel="noopener noreferrer" className="text-purple-300 font-bold underline underline-offset-2 hover:text-purple-200">Ada pertanyaan? Chat WA 💬</a></p></div></section>
